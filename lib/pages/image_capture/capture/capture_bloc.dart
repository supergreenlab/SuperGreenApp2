...
class CaptureBloc extends LegacyBloc<CaptureBlocEvent, CaptureBlocState> {
  ...
  Stream<CaptureBlocState> mapEventToState(CaptureBlocEvent event) async* {
    if (event is CaptureBlocEventInit) {
      ...
      await Future.delayed(
          Duration(milliseconds: 500)); // Adding a small delay for
      ...
      for (File file in files) {
        ...
        try {
          ...
          yield loadingEvent(
              'Copying files ${i + 1}/${files.length}', (i) / (files.length));
          if (ext == 'mov' || ext == 'mp4') {
            if (ext == 'mov') {
              yield loadingEvent(
                  'Converting mov to mp4 ${i + 1}/${files.length}',
                  (i + 0.25) / (files.length));
            ...
            yield loadingEvent('Optimizing pic ${i + 1}/${files.length}',
                (i + 0.75) / (files.length));
            thumbnailPath =
            ...
            await File(jpegPath!)
                .copy(FeedMedias.makeAbsoluteFilePath(filePath));
            thumbnailPath =
                filePath.replaceFirst(fileBaseName, 'thumbnail_$fileBaseName');
            ...
            if (ext == 'png') {
              yield loadingEvent(
                  'Converting png to jpg ${i + 1}/${files.length}',
                  (i + 0.25) / (files.length));
              filePath = '$fileName.jpg';
            }
            await File(FeedMedias.makeAbsoluteFilePath(filePath))
                .writeAsBytes(encodeJpg(image!), flush: true);
            yield loadingEvent('Optimizing pic ${i + 1}/${files.length}',
                (i + 0.75) / (files.length));
            thumbnailPath =
                filePath.replaceFirst(fileBaseName, 'thumbnail_$fileBaseName');
  ...
  Future optimizePictureFromImage(Image image, String to) async {
    ...
    await File(FeedMedias.makeAbsoluteFilePath(to))
        .writeAsBytes(encodeJpg(thumbnail, quality: 50), flush: true);
  }
