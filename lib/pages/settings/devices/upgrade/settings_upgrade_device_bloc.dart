...
class SettingsUpgradeDeviceBlocStateUpgradeError
    extends SettingsUpgradeDeviceBlocState {
...
class SettingsUpgradeDeviceBloc extends LegacyBloc<
    SettingsUpgradeDeviceBlocEvent, SettingsUpgradeDeviceBlocState> {
  ...
  Stream<SettingsUpgradeDeviceBlocState> mapEventToState(
    ...
    if (event is SettingsUpgradeDeviceBlocEventInit) {
      ...
      try {
        start =
            await RelDB.get().devicesDAO.getParam(args.device.id, 'OTA_START');
      } catch (e) {
        hasStart = false;
      }
      if (hasStart) {
        try {
          await DeviceHelper.updateIntParam(args.device, start, 1,
              nRetries: 1, forceLocal: true);
        } catch (e) {
          print(e);
        }
      } else {
        ...
        try {
          await DeviceHelper.updateIntParam(args.device, reboot, 1,
              nRetries: 1, forceLocal: true);
        } catch (e) {
          print(e);
        }
  ...
  Future<void> waitFirmwareUpgraded() async {
    ...
    for (int i = 0; i < nRetries; ++i) {
      ...
      try {
        int value = await DeviceAPI.fetchIntParam(
            args.device.ip, 'OTA_TIMESTAMP',
            timeout: 5, nRetries: 1, auth: auth);
        if (value == ts) {
          break;
        }
      } catch (e, trace) {
