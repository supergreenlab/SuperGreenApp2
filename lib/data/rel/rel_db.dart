...
class RelDB extends _$RelDB {
  ...
  Future<void> runMigrator(Migrator m, int fromVersion) async {
    if (fromVersion == 1) {
      ...
      try {
        await m.addColumn(
            checklistSeeds, checklistSeeds.checklistCollectionServerID);
      } catch (e, trace) {
        Logger.logError(e, trace);
      }
      try {
      ...
      await m.addColumn(devices, devices.needsRefresh);
  ...
  Future<void> runDataChange(int fromVersion) async {
    if (fromVersion == 1) {
      ...
      for (int i = 0; i < timelapses.length; ++i) {
        ...
        await plantsDAO.updateTimelapse(timelapse
            .toCompanion(true)
            .copyWith(settings: Value(meta.toJSON())));
      }
    }
  }
}
