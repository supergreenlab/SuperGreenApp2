...
class BoxHelper {
  static Future removeBoxDevice(Box box,
      {bool removeDevice = false, bool removeScreenDevice = false}) async {
    BoxesCompanion boxC =
        BoxesCompanion(id: Value(box.id), synced: Value(false));
    if (removeDevice && box.device != null && box.deviceBox != null) {
      final Device device = await RelDB.get().devicesDAO.getDevice(box.device!);
      final boxEnabled = await RelDB.get()
          .devicesDAO
          .getParam(box.device!, 'BOX_${box.deviceBox}_ENABLED');
    ...
    if (removeScreenDevice && box.screenDevice != null) {
      ...
      final Param encKey = await RelDB.get()
          .devicesDAO
          .getParam(box.screenDevice!, 'BROKER_ENCKEY');
      await DeviceHelper.updateStringParam(device, encKey, "",
          forceLocal: true);

      final Param token =
          await RelDB.get().devicesDAO.getParam(device.id, 'BROKER_SCRTOKEN');
  ...
  static Future setBoxDevice(Box box,
      {Device? device, int? deviceBox, Device? screenDevice}) async {
    BoxesCompanion boxC =
        BoxesCompanion(id: Value(box.id), synced: Value(false));
    if (device != null &&
        deviceBox != null &&
      ...
      if (hasTimerModule) {
        Param onHourParam = await RelDB.get()
            .devicesDAO
            .getParam(device.id, 'BOX_${deviceBox}_ON_HOUR');
        Param onMinParam = await RelDB.get()
            .devicesDAO
            .getParam(device.id, 'BOX_${deviceBox}_ON_MIN');
        await DeviceHelper.updateHourMinParams(device, onHourParam, onMinParam,
            schedule['ON_HOUR'], schedule['ON_MIN']);

        Param offHourParam = await RelDB.get()
            .devicesDAO
            .getParam(device.id, 'BOX_${deviceBox}_OFF_HOUR');
        Param offMinParam = await RelDB.get()
            .devicesDAO
            .getParam(device.id, 'BOX_${deviceBox}_OFF_MIN');
        await DeviceHelper.updateHourMinParams(device, offHourParam,
            offMinParam, schedule['OFF_HOUR'], schedule['OFF_MIN']);

        final timerTypeParam = await RelDB.get()
            .devicesDAO
            .getParam(device.id, 'BOX_${deviceBox}_TIMER_TYPE');
      ...
      final stateParam =
          await RelDB.get().devicesDAO.getParam(device.id, 'STATE');
      // TODO declare Param enums when possible
      ...
      final boxEnabled = await RelDB.get()
          .devicesDAO
          .getParam(device.id, 'BOX_${deviceBox}_ENABLED');
      await DeviceHelper.updateIntParam(device, boxEnabled, 1);

      boxC =
          boxC.copyWith(device: Value(device.id), deviceBox: Value(deviceBox));
    }
    if (screenDevice != null &&
        screenDevice.isScreen &&
        screenDevice.id != box.screenDevice) {
      boxC = boxC.copyWith(screenDevice: Value(screenDevice.id));

      String key = Uuid().v4();
      final Param encKey = await RelDB.get()
          .devicesDAO
          .getParam(screenDevice.id, 'BROKER_ENCKEY');
      await DeviceHelper.updateStringParam(screenDevice, encKey, key,
          forceLocal: true);

      String scrToken = Uuid().v4();
      final Param token = await RelDB.get()
          .devicesDAO
          .getParam(screenDevice.id, 'BROKER_SCRTOKEN');
      await DeviceHelper.updateStringParam(screenDevice, token, scrToken,
          forceLocal: true);
      boxC =
          boxC.copyWith(screenDeviceToken: Value(scrToken), encKey: Value(key));

      final stateParam =
          await RelDB.get().devicesDAO.getParam(screenDevice.id, 'STATE');
      // TODO declare Param enums when possible
      if (stateParam.ivalue != 2) {
